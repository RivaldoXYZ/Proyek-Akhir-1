...
$queryCount = mysqli_query($con, "SELECT COUNT(*) AS total FROM hotel H JOIN kategori K ON H.kategori_id=K.id WHERE H.akomodasi='penginapan'$starCondition");
...
$queryhotel = mysqli_query($con, "SELECT *, H.nama AS nama_hotel, H.id AS id_hotel FROM hotel H JOIN kategori K ON H.kategori_id=K.id WHERE H.akomodasi='penginapan' $starCondition ORDER BY H.nama LIMIT $limit OFFSET $offset");
?>
...
<body>
    ...
    <div class="pagination">
        ...
        if ($totalPages > 1) {
            // Tautan "previous"
            if ($page > 1) {
                echo '<a href="?page=' . ($page - 1) . '"><i class="fas fa-chevron-left"></i></a>';
            }
            ...
            // Tautan "next"
            if ($page < $totalPages) {
                echo '<a href="?page=' . ($page + 1) . '"><i class="fas fa-chevron-right"></i></a>';
            }
        }
        ?>
    </div>
